...
angular.module("openeis-ui",["openeis-ui.auth","openeis-ui.projects","openeis-ui.templates","ngAnimate","ngRoute"]).constant("API_URL","/api").constant("ANON_HOME","/").constant("AUTH_HOME","/projects").config(["$routeProvider","$locationProvider","$httpProvider",function(a,b,c){a.otherwise({templateUrl:"partials/404.html"}),b.html5Mode(!0),c.defaults.xsrfHeaderName="X-CSRFToken",c.defaults.xsrfCookieName="csrftoken"}]),angular.module("openeis-ui.auth",["ngResource","ngRoute"]).provider("authRoute",["$routeProvider",function(a){this.whenAnon=function(b,c){return c.resolve=c.resolve||{},angular.extend(c.resolve,{anon:["Auth",function(a){return a.requireAnon()}]}),a.when(b,c),this},this.whenAuth=function(b,c){return c.resolve=c.resolve||{},angular.extend(c.resolve,{auth:["Auth",function(a){return a.requireAuth()}]}),a.when(b,c),this},this.$get=a.$get}]).config(["authRouteProvider",function(a){a.whenAnon("/",{controller:"LoginCtrl",templateUrl:"partials/login.html"}).whenAnon("/sign-up",{controller:"SignUpCtrl",templateUrl:"partials/signup.html"}).whenAnon("/recovery",{controller:"RecoveryCtrl",templateUrl:"partials/recovery.html"}).whenAuth("/account",{controller:"AccountCtrl",templateUrl:"partials/account.html"})}]).service("Auth",["$resource","API_URL","$q","ANON_HOME","AUTH_HOME","$location",function(a,b,c,d,e,f){var g=null,h="Anonymous",i=a(b+"/auth");this.username=function(){return h},this.init=function(){var a=c.defer();return i.get(function(b){g=!0,h=b.username,a.resolve()},function(){g=!1,a.resolve()}),a.promise},this.logIn=function(a){var b=c.defer();return i.save(a,function(a){g=!0,h=a.username,b.resolve()},function(a){b.reject(a)}),b.promise},this.logOut=function(){var a=c.defer();return i.delete(function(){g=!1,h="Anonymous",a.resolve()},function(b){a.reject(b)}),a.promise},this.requireAnon=function(){function a(){g?(f.url(e),b.reject()):b.resolve()}var b=c.defer();return null===g?this.init().then(function(){a()}):a(),b.promise},this.requireAuth=function(){function a(){g?b.resolve():(f.url(d),b.reject())}var b=c.defer();return null===g?this.init().then(function(){a()}):a(),b.promise}}]).controller("LoginCtrl",["$scope","$location","Auth","AUTH_HOME",function(a,b,c,d){a.logIn=function(){c.logIn({username:a.form.username,password:a.form.password}).then(function(){b.url(d)},function(b){a.form.error=b.status})},a.clearError=function(){a.form.error=null}}]).controller("SignUpCtrl",["$scope","$location","Auth","AUTH_HOME",function(a){a.form={},a.signUp=function(){console.log(a.form)}}]).controller("RecoveryCtrl",["$scope","$location","Auth",function(a){a.form={},a.submit=function(){console.log(a.form)}}]).controller("AccountCtrl",["$scope","Auth",function(a,b){a.form={changed:!1,submit:function(){console.log(a.account)}},a.account={username:b.username(),email:b.username()+".email@example.com"},a.$watchCollection("account",function(b,c){b!==c&&(a.form.changed=!0)})}]).controller("TopBarCtrl",["$scope","Auth","$location","ANON_HOME",function(a,b,c,d){a.username=b.username(),a.logOut=function(){b.logOut().then(function(){c.url(d)})}}]),angular.module("openeis-ui.file",[]).directive("fileUpload",["$parse",function(a){return{restrict:"E",template:'<input type="file"><button disabled>Upload</button>',compile:function(b,c){return clickFn=a(c.fileUploadClick),function(a,b){var c=b.find("input"),d=b.find("button");c.on("change",function(){c[0].files.length?d.prop("disabled",!1):d.prop("disabled",!0)}),d.on("click",function(b){a.$apply(function(){clickFn(a,{$event:b,fileInput:c})})})}}}}]).filter("bytes",function(){return function(a,b){if(isNaN(parseFloat(a))||!isFinite(a))return"--";"undefined"==typeof b&&(b=0);var c=["B","KB","MB","GB","TB","PB"],d=Math.floor(Math.log(a)/Math.log(1024));return(a/Math.pow(1024,Math.floor(d))).toFixed(b)+" "+c[d]}}),angular.module("openeis-ui.projects",["openeis-ui.auth","openeis-ui.file","ngResource","angularFileUpload"]).config(["authRouteProvider",function(a){a.whenAuth("/projects",{controller:"ProjectsCtrl",templateUrl:"partials/projects.html",resolve:{projects:["Projects",function(a){return a.query()}]}}).whenAuth("/projects/:projectId",{controller:"ProjectCtrl",templateUrl:"partials/project.html",resolve:{project:["Projects","$route",function(a,b){return a.get(b.current.params.projectId)}],projectFiles:["ProjectFiles","$route",function(a,b){return a.query(b.current.params.projectId)}]}})}]).factory("Projects",["$resource","API_URL",function(a,b){var c=a(b+"/projects/:projectId",{projectId:"@id"},{create:{method:"POST"},save:{method:"PUT"}});return{get:function(a){return c.get({projectId:a}).$promise},query:function(){return c.query().$promise},create:function(a){return c.create(a).$promise}}}]).factory("ProjectFiles",["$resource","API_URL","$http",function(a,b,c){var d=a(b+"/files/:fileId",{fileId:"@id"});return{get:function(a){return d.get({fileId:a}).$promise},query:function(a){return d.query({project:a}).$promise},head:function(a){return c({method:"GET",url:b+"/files/"+a+"/top",transformResponse:angular.fromJson})}}}]).controller("ProjectsCtrl",["$scope","projects","Projects",function(a,b,c){a.projects=b,a.newProject={name:"",create:function(){c.create({name:a.newProject.name}).then(function(b){a.newProject.name="",a.projects.push(b)})}},a.renameProject=function(b){var c=prompt("New project name:");c&&c.length&&(a.projects[b].name=c,a.projects[b].$save(function(c){a.projects[b]=c}))},a.deleteProject=function(b){a.projects[b].$delete(function(){a.projects.splice(b,1)})}}]).controller("ProjectCtrl",["$scope","project","projectFiles","$upload","API_URL","ProjectFiles",function(a,b,c,d,e,f){function g(b){a.modal={show:!0,file:b}}a.project=b,a.projectFiles=c,a.upload=function(c){angular.forEach(c[0].files,function(h){d.upload({url:e+"/projects/"+b.id+"/add_file",file:h}).then(function(b){f.head(b.data.id).then(function(a){a.data.has_header&&(a.data.header=a.data.rows.shift()),b.data.head=a.data,g(b.data)}),f.get(b.data.id).then(function(b){a.projectFiles.push(b)}),c.val("").triggerHandler("change")})})},a.deleteFile=function(b){a.projectFiles[b].$delete(function(){a.projectFiles.splice(b,1)})}}]),angular.module("openeis-ui.templates",[]).run(["$templateCache",function(a){"use strict";a.put("partials/404.html","<h2>404 Page not found</h2>"),a.put("partials/account.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=container><ul class=breadcrumbs><li>Account</li></ul><div class="view view--account"><h2>Account</h2><form ng-submit=form.submit()><p><label>Username</label><input ng-model=account.username disabled></p><p><label for=email>Email address</label><input id=email ng-model=account.email></p><p><label for=new-password>New password</label><input id=new-password placeholder="New password" ng-model=account.newPassword><input id=new-password-confirm placeholder="Confirm new password" ng-model=account.newPasswordConfirm></p><p><label for=current-password>Current password</label><input id=current-password placeholder="Current password" ng-disabled=!form.changed ng-model=account.currentPassword></p><button ng-disabled=!form.changed>Update account</button></form></div></div>'),a.put("partials/login.html",'<div class=container><div class="view view--login"><h1>OpenEIS</h1><form ng-submit=logIn()><div class=form__group><input type=text placeholder=Username required ng-model=form.username ng-change=clearError()></div><div class=form__group><input type=password placeholder=Password required ng-model=form.password ng-change=clearError()></div><button type=submit>Log in</button><div class=links><a href=sign-up>Need an account?</a><br><a href=recovery>Can\'t log in?</a></div><div class="alert alert--error" ng-if=form.error ng-switch=form.error><span ng-switch-when=403>Authentication failed.</span> <span ng-switch-default="">Unknown error occurred. ({{form.error}})</span></div></form></div></div>'),a.put("partials/project.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=container><ul class=breadcrumbs><li><a href=projects>Projects</a></li><li>{{project.name}}</li></ul><div class="view view--project"><h2>{{project.name}}</h2><div ng-if=projectFiles.length><h3>Project files</h3><table><thead><tr><th>Name</th><th>Size</th><th>Download</th><th>Delete</th></tr></thead><tbody><tr ng-repeat="file in projectFiles"><td>{{file.file}}</td><td>{{file.size | bytes}}</td><td><a href={{file.download_url}} target=_self>Download</a></td><td><a ng-click=deleteFile($index)>Delete</a></td></tr></tbody></table></div><label>Upload file</label><file-upload file-upload-click=upload(fileInput)></file-upload></div></div><div class=modal ng-show=modal.show><div class=modal__dialog><h4>{{modal.file.file}}</h4><table><thead ng-if=modal.file.head.header.length><tr><th ng-repeat="header in modal.file.head.header">{{header}}</th></tr></thead><tbody><tr ng-repeat="row in modal.file.head.rows"><td ng-repeat="col in row">{{col}}</td></tr></tbody></table><button ng-click="modal.show = false">Close</button></div></div>'),a.put("partials/projects.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=container><ul class=breadcrumbs><li>Projects</li></ul><div class="view view--projects"><h2>Projects</h2><ul ng-if=projects.length><li ng-repeat="project in projects"><a ng-href=projects/{{project.id}}>{{project.name}}</a> &mdash; <a ng-click=renameProject($index)>Rename</a> &mdash; <a ng-click=deleteProject($index)>Delete</a></li></ul><form ng-submit=newProject.create()><label>Create new project</label><input type=text placeholder=Name ng-model=newProject.name required><button type=submit>Create</button></form></div></div>'),a.put("partials/recovery.html",'<div class=container><div class="view view--recovery"><h1>OpenEIS</h1><form class="panel radius" ng-submit=submit()><h2>Recover account</h2><div class=form__group><input id=name type=text placeholder="Username or email address" required ng-model=form.id></div><button type=submit>Submit</button></form></div></div>'),a.put("partials/signup.html",'<div class=container><div class="view view--signup"><h1>OpenEIS</h1><form class="panel radius" ng-submit=signUp()><h2>Get an account</h2><div class=form__group><label for=name>Name</label><input id=name type=text required ng-model=form.name></div><div class=form__group><label for=email>Email address</label><input id=email type=email required ng-model=form.email></div><div class=form__group><label for=username>Username</label><input id=username type=text autocomplete=off required ng-model=form.username></div><div class=form__group><label for=password>Password</label><input id=password type=password autocomplete=off placeholder="Enter password" required ng-model=form.password><input type=password autocomplete=off placeholder="Re-enter password" required ng-model=form.passwordconfirm></div><button type=submit>Sign up</button></form></div></div>'),a.put("partials/topbar.html","<div class=topbar ng-controller=TopBarCtrl><nav class=container><h1><a href=projects>OpenEIS</a></h1><ul><li class=divider></li><li><a href=projects>Projects</a></li><li class=divider></li><li><a href=account>Account: <strong>{{username}}</strong></a><ul class=dropdown><li><a ng-click=logOut()>Log out</a></li></ul></li><li class=divider></li></ul></nav></div>")}]);