...
angular.module("openeis-ui",["openeis-ui.auth","openeis-ui.projects","openeis-ui.templates","ngAnimate","ngRoute"]).constant("API_URL","/api").constant("LOGIN_PAGE","/").constant("AUTH_HOME","/projects").config(["$routeProvider","$locationProvider","$httpProvider",function(a,b,c){a.otherwise({templateUrl:"partials/404.html"}),b.html5Mode(!0),c.defaults.xsrfHeaderName="X-CSRFToken",c.defaults.xsrfCookieName="csrftoken"}]),angular.module("openeis-ui.auth",["ngResource","ngRoute"]).provider("authRoute",["$routeProvider",function(a){this.whenAnon=function(b,c){return c.resolve=c.resolve||{},angular.extend(c.resolve,{anon:["Auth",function(a){return a.requireAnon()}]}),a.when(b,c),this},this.whenAuth=function(b,c){return c.resolve=c.resolve||{},angular.extend(c.resolve,{auth:["Auth",function(a){return a.requireAuth()}]}),a.when(b,c),this},this.$get=a.$get}]).config(["authRouteProvider",function(a){a.whenAnon("/",{controller:"LoginCtrl",templateUrl:"partials/login.html"}).whenAnon("/sign-up",{controller:"SignUpCtrl",templateUrl:"partials/signup.html"}).whenAnon("/recovery",{controller:"RecoveryCtrl",templateUrl:"partials/recovery.html"}).whenAuth("/account",{controller:"AccountCtrl",templateUrl:"partials/account.html"})}]).service("Auth",["$resource","API_URL","$q","LOGIN_PAGE","AUTH_HOME","$location",function(a,b,c,d,e,f){var g=this,h=null,i=a(b+"/account",null,{create:{method:"POST"},update:{method:"PATCH"}}),j=a(b+"/account/login"),k=a(b+"/account/change_password"),l=a(b+"/account/password_reset",null,{put:{method:"PUT"}}),m=null;g.account=function(){return h},g.accountCreate=function(a){return i.create(a).$promise},g.accountUpdate=function(a){return i.update(a).$promise},g.accountPassword=function(a){return k.save(a).$promise},g.accountRecover1=function(a){return l.save({username_or_email:a}).$promise},g.accountRecover2=function(a){return l.put(a).$promise},g.init=function(){var a=c.defer();return i.get(function(b){h=b,a.resolve()},function(){h=!1,a.resolve()}),a.promise},g.logIn=function(a){var b=c.defer();return j.save(a,function(){g.init().then(function(){null!==m?(f.url(m),m=null):f.url(e),b.resolve()})},function(a){b.reject(a)}),b.promise},g.logOut=function(){var a=c.defer();return j.delete(function(){h=!1,f.url(d),a.resolve()},function(b){a.reject(b)}),a.promise},g.requireAnon=function(){function a(){h?(f.url(e),b.reject()):b.resolve()}var b=c.defer();return null===h?g.init().then(function(){a()}):a(),b.promise},g.requireAuth=function(){function a(){h?b.resolve():(m=f.url(),f.url(d),b.reject())}var b=c.defer();return null===h?g.init().then(function(){a()}):a(),b.promise}}]).controller("LoginCtrl",["$scope","Auth",function(a,b){a.logIn=function(){b.logIn({username:a.form.username,password:a.form.password}).catch(function(b){a.form.error=b.status})},a.clearError=function(){a.form.error=null}}]).controller("SignUpCtrl",["$scope","$location","Auth","AUTH_HOME",function(a,b,c){a.form={},a.submit=function(){c.accountCreate(a.account).then(function(){c.logIn({username:a.account.username,password:a.account.password}).catch(function(b){a.form.error=b.status})},function(b){400===b.status&&angular.forEach(b.data,function(a,c){angular.isArray(a)&&(b.data[c]=a.join("<br>"))}),a.form.error=b})},a.clearError=function(){a.form.error=null}}]).controller("RecoveryCtrl",["$scope","Auth","$routeParams",function(a,b,c){a.form={stage:1},c.username&&c.code?(a.form.stage=2,a.recovery={username:c.username,code:c.code}):a.recovery={},a.submit=function(){switch(a.form.stage){case 1:return void b.accountRecover1(a.recovery.id).then(function(){a.form.success="Email with temporary password reset link sent.",a.form.stage=!1,a.clearError()},function(b){404===b.status&&(b.data="Account with specified username or email address not found."),a.form.error=b});case 2:return a.recovery.password!==a.recovery.passwordConfirm?void(a.form.error={status:400,data:"Passwords do not match."}):void b.accountRecover2(a.recovery).then(function(){a.form.success="Password updated.",a.form.stage=!1,a.clearError()},function(b){404===b.status&&(b.data="Invalid password reset link."),a.form.error=b})}},a.clearError=function(){a.form.error=!1}}]).controller("AccountCtrl",["$scope","Auth","$timeout",function(a,b,c){a.account=b.account(),a.profile={clearAlerts:function(){a.profile.success=!1,a.profile.error=!1},update:function(){a.profile.clearAlerts(),b.accountUpdate({first_name:a.account.first_name,last_name:a.account.last_name,email:a.account.email}).then(function(){a.profile.success=!0,c(a.profile.clearAlerts,2e3)},function(b){a.profile.error=b.status})}},a.password={clearAlerts:function(){a.password.mismatch=!1,a.password.success=!1,a.password.error=!1},update:function(){return a.password.clearAlerts(),a.password.new!==a.password.newConfirm?void(a.password.mismatch=!0):void b.accountPassword({old_password:a.password.current,new_password:a.password.new}).then(function(){a.password.success=!0,a.password.current="",a.password.new="",a.password.newConfirm="",c(a.password.clearAlerts,2e3)},function(b){400===b.status&&angular.forEach(b.data,function(a,c){angular.isArray(a)&&(b.data[c]=a.join("<br>"))}),a.password.error=b})}}}]).controller("TopBarCtrl",["$scope","Auth",function(a,b){a.account=b.account(),a.logOut=function(){b.logOut()}}]),angular.module("openeis-ui.file",[]).directive("fileUpload",["$parse",function(a){return{restrict:"E",template:'<input type="file"><button disabled>Upload</button>',compile:function(b,c){return clickFn=a(c.fileUploadClick),function(a,b){var c=b.find("input"),d=b.find("button");c.on("change",function(){c[0].files.length?d.prop("disabled",!1):d.prop("disabled",!0)}),d.on("click",function(b){a.$apply(function(){clickFn(a,{$event:b,fileInput:c})})})}}}}]).filter("bytes",function(){return function(a,b){if(isNaN(parseFloat(a))||!isFinite(a))return"--";"undefined"==typeof b&&(b=0);var c=["B","KB","MB","GB","TB","PB"],d=Math.floor(Math.log(a)/Math.log(1024));return(a/Math.pow(1024,Math.floor(d))).toFixed(b)+" "+c[d]}}),angular.module("openeis-ui.projects",["openeis-ui.auth","openeis-ui.file","ngResource","angularFileUpload"]).config(["authRouteProvider",function(a){a.whenAuth("/projects",{controller:"ProjectsCtrl",templateUrl:"partials/projects.html",resolve:{projects:["Projects",function(a){return a.query()}]}}).whenAuth("/projects/:projectId",{controller:"ProjectCtrl",templateUrl:"partials/project.html",resolve:{project:["Projects","$route",function(a,b){return a.get(b.current.params.projectId)}],projectFiles:["ProjectFiles","$route",function(a,b){return a.query(b.current.params.projectId)}]}})}]).factory("Projects",["$resource","API_URL",function(a,b){var c=a(b+"/projects/:projectId",{projectId:"@id"},{create:{method:"POST"},save:{method:"PUT"}});return{get:function(a){return c.get({projectId:a}).$promise},query:function(){return c.query().$promise},create:function(a){return c.create(a).$promise}}}]).factory("ProjectFiles",["$resource","API_URL","$http",function(a,b,c){var d=a(b+"/files/:fileId",{fileId:"@id"});return{get:function(a){return d.get({fileId:a}).$promise},query:function(a){return d.query({project:a}).$promise},head:function(a){return c({method:"GET",url:b+"/files/"+a+"/top",transformResponse:angular.fromJson})}}}]).controller("ProjectsCtrl",["$scope","projects","Projects",function(a,b,c){a.projects=b,a.newProject={name:"",create:function(){c.create({name:a.newProject.name}).then(function(b){a.newProject.name="",a.projects.push(b)})}},a.renameProject=function(b){var c=prompt("New project name:");c&&c.length&&(a.projects[b].name=c,a.projects[b].$save(function(c){a.projects[b]=c}))},a.deleteProject=function(b){a.projects[b].$delete(function(){a.projects.splice(b,1)})}}]).controller("ProjectCtrl",["$scope","project","projectFiles","$upload","API_URL","ProjectFiles",function(a,b,c,d,e,f){function g(b){a.modal={show:!0,file:b}}a.project=b,a.projectFiles=c,a.upload=function(c){angular.forEach(c[0].files,function(h){d.upload({url:e+"/projects/"+b.id+"/add_file",file:h}).then(function(b){f.head(b.data.id).then(function(a){a.data.has_header&&(a.data.header=a.data.rows.shift()),b.data.head=a.data,g(b.data)}),f.get(b.data.id).then(function(b){a.projectFiles.push(b)}),c.val("").triggerHandler("change")})})},a.deleteFile=function(b){a.projectFiles[b].$delete(function(){a.projectFiles.splice(b,1)})}}]),angular.module("openeis-ui.templates",[]).run(["$templateCache",function(a){"use strict";a.put("partials/404.html","<h2>404 Page not found</h2>"),a.put("partials/account.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=container><ul class=breadcrumbs><li>Account</li></ul><div class="view view--account"><h2>Account</h2><h3>Profile</h3><form ng-submit=profile.update()><div class=form__group><label>Username</label><input ng-model=account.username disabled></div><div class=form__group><label for=name>Name</label><input id=name type=text ng-model=account.first_name></div><div class=form__group><label for=email>Email address</label><input id=email type=email ng-model=account.email></div><div class=form__group><button>Update profile</button></div><div class="alert alert--success animated--slow-out" ng-if=profile.success>Profile updated.</div><div class="alert alert--error" ng-if=profile.error ng-switch=profile.error><span ng-switch-default="">Unknown error occurred. ({{profile.error}})</span></div></form><h3>Password</h3><form ng-submit=password.update()><div class=form__group><label for=current-password>Current password</label><input id=current-password type=password autocomplete=off ng-trim=false ng-change=password.clearAlerts() required ng-model=password.current><small class="help help--error" ng-if=password.error.data.old_password>{{password.error.data.old_password}}</small><br><label for=new-password>New password</label><input id=new-password type=password autocomplete=off required ng-trim=false ng-change=password.clearAlerts() ng-model=password.new><small class="help help--error" ng-if=password.error.data.new_password>{{password.error.data.new_password}}</small><br><label for=confirm-new-password>Confirm new password</label><input id=confirm-new-password type=password autocomplete=off required ng-trim=false ng-change=password.clearAlerts() ng-model=password.newConfirm><small class="help help--error" ng-if=password.mismatch>New passwords do not match.</small><br></div><div class=form__group><button ng-click=updatePassword()>Update password</button></div><div class="alert alert--success animated--slow-out" ng-if=password.success>Password updated.</div></form></div></div>'),a.put("partials/login.html",'<div class=container><div class="view view--login"><h1>OpenEIS</h1><form ng-submit=logIn()><div class=form__group><input type=text placeholder=Username required ng-model=form.username ng-change=clearError()></div><div class=form__group><input type=password placeholder=Password required ng-model=form.password ng-trim=false ng-change=clearError()></div><div class=form__group><button type=submit>Log in</button><div class=links><a href=sign-up>Need an account?</a><br><a href=recovery>Can\'t log in?</a></div></div><div class="alert alert--error" ng-if=form.error ng-switch=form.error><span ng-switch-when=403>Authentication failed.</span> <span ng-switch-default="">Unknown error occurred. ({{form.error}})</span></div></form></div></div>'),a.put("partials/project.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=container><ul class=breadcrumbs><li><a href=projects>Projects</a></li><li>{{project.name}}</li></ul><div class="view view--project"><h2>{{project.name}}</h2><div ng-if=projectFiles.length><h3>Project files</h3><table><thead><tr><th>Name</th><th>Size</th><th>Download</th><th>Delete</th></tr></thead><tbody><tr ng-repeat="file in projectFiles"><td>{{file.file}}</td><td>{{file.size | bytes}}</td><td><a href={{file.download_url}} target=_self>Download</a></td><td><a ng-click=deleteFile($index)>Delete</a></td></tr></tbody></table></div><label>Upload file</label><file-upload file-upload-click=upload(fileInput)></file-upload></div></div><div class=modal ng-show=modal.show><div class=modal__dialog><h4>{{modal.file.file}}</h4><table><thead ng-if=modal.file.head.header.length><tr><th ng-repeat="header in modal.file.head.header">{{header}}</th></tr></thead><tbody><tr ng-repeat="row in modal.file.head.rows"><td ng-repeat="col in row">{{col}}</td></tr></tbody></table><button ng-click="modal.show = false">Close</button></div></div>'),a.put("partials/projects.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=container><ul class=breadcrumbs><li>Projects</li></ul><div class="view view--projects"><h2>Projects</h2><ul ng-if=projects.length><li ng-repeat="project in projects"><a ng-href=projects/{{project.id}}>{{project.name}}</a> &mdash; <a ng-click=renameProject($index)>Rename</a> &mdash; <a ng-click=deleteProject($index)>Delete</a></li></ul><form ng-submit=newProject.create()><label>Create new project</label><input type=text placeholder=Name ng-model=newProject.name required><button type=submit>Create</button></form></div></div>'),a.put("partials/recovery.html",'<div class=container><div class="view view--recovery"><h1><a href="/">OpenEIS</a></h1><form class=panel ng-submit=submit()><h2>Recover account</h2><div class=form__group ng-if="form.stage === 1"><label for=recoveryId>Username or email address</label><input id=recoveryId type=text required ng-change=clearError() ng-trim=false ng-model=recovery.id></div><div class=form__group ng-if="form.stage === 2"><label>Username</label><input type=text disabled ng-model=recovery.username></div><div class=form__group ng-if="form.stage === 2"><label for=password>New password</label><input id=password type=password required ng-trim=false ng-model=recovery.password><br><label for=password>Confirm new password</label><input type=password required ng-trim=false ng-model=recovery.passwordConfirm></div><div class=form__group ng-if=form.stage><button type=submit>Submit</button></div><div class="alert alert--success" ng-if=form.success>{{form.success}}</div><div class="alert alert--error" ng-if=form.error ng-switch=form.error.status><span ng-switch-when=400>{{form.error.data}}</span> <span ng-switch-when=404>{{form.error.data}}</span> <span ng-switch-default="">Unknown error occurred. ({{form.error}})</span></div></form></div></div>'),a.put("partials/signup.html",'<div class=container><div class="view view--signup"><h1>OpenEIS</h1><form class=panel ng-submit=submit()><h2>Get an account</h2><div class=form__group><label for=name>Name</label><input id=name type=text ng-model=account.first_name><small class="help help--error" ng-if=form.error.data.first_name>{{form.error.data.first_name}}</small></div><div class=form__group><label for=email>Email address</label><input id=email type=email required ng-change="form.error.data.email = false" ng-model=account.email><small class="help help--error" ng-if=form.error.data.email>{{form.error.data.email}}</small></div><div class=form__group><label for=username>Username</label><input id=username type=text autocomplete=off required ng-change="form.error.data.username = false" ng-model=account.username><small class="help help--error" ng-if=form.error.data.username>{{form.error.data.username}}</small></div><div class=form__group><label for=password>Password</label><input id=password type=password autocomplete=off required ng-trim=false ng-change="form.error.data.password = false" ng-model=account.password><small class="help help--error" ng-if=form.error.data.password>{{form.error.data.password}}</small></div><div class=form__group><button type=submit>Sign up</button></div><div class="alert alert--error" ng-if="form.error.status && form.error.status !== 400">Unknown error occurred. ({{form.error.status}})</div></form></div></div>'),a.put("partials/topbar.html","<div class=topbar ng-controller=TopBarCtrl><nav class=container><h1><a href=projects>OpenEIS</a></h1><ul><li><a href=projects>Projects</a></li><li><a href=account>Account: <strong>{{account.username}}</strong></a><ul class=dropdown><li><a ng-click=logOut()>Log out</a></li></ul></li></ul></nav></div>")}]);