...
angular.module("openeis-ui",["openeis-ui.auth","openeis-ui.projects","openeis-ui.templates","ngAnimate","ngRoute"]).constant("API_URL","/api").constant("ANON_HOME","/").constant("AUTH_HOME","/projects").config(["$routeProvider","$locationProvider","$httpProvider",function(a,b,c){a.otherwise({templateUrl:"partials/404.html"}),b.html5Mode(!0),c.defaults.xsrfHeaderName="X-CSRFToken",c.defaults.xsrfCookieName="csrftoken"}]),angular.module("openeis-ui.auth",["ngResource","ngRoute"]).provider("authRoute",["$routeProvider",function(a){this.whenAnon=function(b,c){return c.resolve=c.resolve||{},angular.extend(c.resolve,{anon:["Auth",function(a){return a.requireAnon()}]}),a.when(b,c),this},this.whenAuth=function(b,c){return c.resolve=c.resolve||{},angular.extend(c.resolve,{auth:["Auth",function(a){return a.requireAuth()}]}),a.when(b,c),this},this.$get=a.$get}]).config(["authRouteProvider",function(a){a.whenAnon("/",{controller:"LoginCtrl",templateUrl:"partials/login.html"}).whenAnon("/sign-up",{controller:"SignUpCtrl",templateUrl:"partials/signup.html"})}]).service("Auth",["$resource","API_URL","$q","ANON_HOME","AUTH_HOME","$location",function(a,b,c,d,e,f){var g=null,h="Anonymous",i=a(b+"/auth");this.username=function(){return h},this.init=function(){var a=c.defer();return i.get(function(b){g=!0,h=b.username,a.resolve()},function(){g=!1,a.resolve()}),a.promise},this.logIn=function(a){var b=c.defer();return i.save(a,function(a){g=!0,h=a.username,b.resolve()},function(a){b.reject(a)}),b.promise},this.logOut=function(){var a=c.defer();return i.delete(function(){g=!1,h="Anonymous",a.resolve()},function(b){a.reject(b)}),a.promise},this.requireAnon=function(){function a(){g?(f.url(e),b.reject()):b.resolve()}var b=c.defer();return null===g?this.init().then(function(){a()}):a(),b.promise},this.requireAuth=function(){function a(){g?b.resolve():(f.url(d),b.reject())}var b=c.defer();return null===g?this.init().then(function(){a()}):a(),b.promise}}]).controller("LoginCtrl",["$scope","$location","Auth","AUTH_HOME",function(a,b,c,d){a.form={},a.form.logIn=function(){c.logIn({username:a.form.username,password:a.form.password}).then(function(){b.url(d)},function(b){switch(b.status){case 403:a.form.error="Authentication failed.";break;default:a.form.error="Unknown error occurred."}})}}]).controller("SignUpCtrl",["$scope","$location","Auth","AUTH_HOME",function(a){a.form={},a.signUp=function(){console.log(a.form)}}]).controller("TopBarCtrl",["$scope","Auth","$location","ANON_HOME",function(a,b,c,d){a.username=b.username(),a.logOut=function(){b.logOut().then(function(){c.url(d)})}}]),angular.module("openeis-ui.file",[]).directive("fileUpload",["$parse",function(a){return{restrict:"E",template:'<input type="file"><button disabled>Upload</button>',compile:function(b,c){return clickFn=a(c.fileUploadClick),function(a,b){var c=b.find("input"),d=b.find("button");c.on("change",function(){c[0].files.length?d.prop("disabled",!1):d.prop("disabled",!0)}),d.on("click",function(b){a.$apply(function(){clickFn(a,{$event:b,fileInput:c})})})}}}}]).filter("bytes",function(){return function(a,b){if(isNaN(parseFloat(a))||!isFinite(a))return"--";"undefined"==typeof b&&(b=0);var c=["B","KB","MB","GB","TB","PB"],d=Math.floor(Math.log(a)/Math.log(1024));return(a/Math.pow(1024,Math.floor(d))).toFixed(b)+" "+c[d]}}),angular.module("openeis-ui.projects",["openeis-ui.auth","openeis-ui.file","ngResource","angularFileUpload"]).config(["authRouteProvider",function(a){a.whenAuth("/projects",{controller:"ProjectsCtrl",templateUrl:"partials/projects.html",resolve:{projects:["Projects",function(a){return a.query()}]}}).whenAuth("/projects/:projectId",{controller:"ProjectCtrl",templateUrl:"partials/project.html",resolve:{project:["Projects","$route",function(a,b){return a.get(b.current.params.projectId)}],projectFiles:["ProjectFiles","$route",function(a,b){return a.query(b.current.params.projectId)}]}})}]).factory("Projects",["$resource","API_URL",function(a,b){var c=a(b+"/projects/:projectId",{projectId:"@id"},{create:{method:"POST"},save:{method:"PUT"}});return{get:function(a){return c.get({projectId:a}).$promise},query:function(){return c.query().$promise},create:function(a){return c.create(a).$promise}}}]).factory("ProjectFiles",["$resource","API_URL","$http",function(a,b,c){var d=a(b+"/files/:fileId",{fileId:"@id"});return{get:function(a){return d.get({fileId:a}).$promise},query:function(a){return d.query({project:a}).$promise},head:function(a){return c({method:"GET",url:b+"/files/"+a+"/top",transformResponse:angular.fromJson})}}}]).controller("ProjectsCtrl",["$scope","projects","Projects",function(a,b,c){a.projects=b,a.newProject={name:"",create:function(){c.create({name:a.newProject.name}).then(function(b){a.newProject.name="",a.projects.push(b)})}},a.renameProject=function(b){var c=prompt("New project name:");c&&c.length&&(a.projects[b].name=c,a.projects[b].$save(function(c){a.projects[b]=c}))},a.deleteProject=function(b){a.projects[b].$delete(function(){a.projects.splice(b,1)})}}]).controller("ProjectCtrl",["$scope","project","projectFiles","$modal","$upload","API_URL","ProjectFiles",function(a,b,c,d,e,f,g){function h(a){var b=d.open({templateUrl:"partials/addfile.html",controller:"FileModalCtrl",resolve:{file:function(){return a}}});b.result.then(function(a){console.log(a)},function(a){console.log(a)})}a.project=b,a.projectFiles=c,a.upload=function(c){angular.forEach(c[0].files,function(d){e.upload({url:f+"/projects/"+b.id+"/add_file",file:d}).then(function(b){g.head(b.data.id).then(function(a){a.data.has_header&&(a.data.header=a.data.rows.shift()),b.data.head=a.data,h(b.data)}),g.get(b.data.id).then(function(b){a.projectFiles.push(b)}),c.val("").triggerHandler("change")})})},a.deleteFile=function(b){a.projectFiles[b].$delete(function(){a.projectFiles.splice(b,1)})}}]).controller("FileModalCtrl",["$scope","$modalInstance","file",function(a,b,c){a.file=c,a.ok=function(){b.close("Clicked OK.")},a.cancel=function(){b.dismiss("Cancelled.")}}]),angular.module("openeis-ui.templates",[]).run(["$templateCache",function(a){"use strict";a.put("partials/404.html","<h2>404 Page not found</h2>"),a.put("partials/addfile.html",'<h3>First rows of {{file.file}}</h3><table><thead ng-if=file.head.header.length><tr><th ng-repeat="header in file.head.header">{{header}}</th></tr></thead><tbody><tr ng-repeat="row in file.head.rows"><td ng-repeat="col in row">{{col}}</td></tr></tbody></table><button ng-click=ok()>OK</button> <a class=close-reveal-modal ng-click=cancel()>&#215;</a>'),a.put("partials/login.html",'<div class=row style="padding-top: 200px"><div class="small-8 columns"><h1 style="font-size: 7.5rem; line-height: 1.5">OpenEIS</h1></div><div class="small-4 columns"><form class="panel radius" ng-submit=form.logIn()><input type=text placeholder=Username required ng-model=form.username ng-change="form.error = false"><input type=password placeholder=Password required ng-model=form.password ng-change="form.error = false"><p class=right style="width: 140px; line-height: 3"><a href=sign-up>Create new account</a></p><button type=submit>Log in</button><p class="alert-box warning radius" ng-if=form.error>{{form.error}}</p></form></div></div>'),a.put("partials/project.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=row style="margin-top: 1em"><div class=columns><ul class=breadcrumbs><li><a href=projects>Dashboard</a></li><li>{{project.name}}</li></ul></div></div><div class=row><div class=columns><h2>{{project.name}}</h2><div ng-if=projectFiles.length><h3>Project files</h3><table><thead><tr><th>Name</th><th>Size</th><th>Download</th><th>Delete</th></tr></thead><tbody><tr ng-repeat="file in projectFiles"><td>{{file.file}}</td><td>{{file.size | bytes}}</td><td><a href={{file.download_url}} target=_self>Download</a></td><td><a ng-click=deleteFile($index)>Delete</a></td></tr></tbody></table></div><h3>Add file</h3><file-upload file-upload-click=upload(fileInput)></file-upload></div></div>'),a.put("partials/projects.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=row style="margin-top: 1em"><div class=columns><ul class=breadcrumbs><li>Dashboard</li></ul></div></div><div class=row><div class=columns><h2>Dashboard</h2></div></div><div class=row id=main><div class=columns><ul ng-if=projects.length><li ng-repeat="project in projects"><a ng-href=projects/{{project.id}}>{{project.name}}</a> &mdash; <a ng-click=renameProject($index)>Rename</a> &mdash; <a ng-click=deleteProject($index)>Delete</a></li></ul><form ng-submit=newProject.create()><input type=text placeholder="Name of new project" ng-model=newProject.name required><button type=submit>Create new project</button></form></div></div>'),a.put("partials/signup.html",'<div class=row style="padding-top: 60px"><div class="small-6 small-offset-3 columns end"><h1>OpenEIS</h1><form class="panel radius" ng-submit=signUp()><h2>Sign up</h2><label>Name</label><input type=text required ng-model=form.name><label>Email address</label><input type=email required ng-model=form.email><label>Username</label><input type=text autocomplete=off required ng-model=form.username><label>Password</label><input type=password autocomplete=off placeholder="Enter password" required ng-model=form.password><input type=password autocomplete=off placeholder="Re-enter password" required ng-model=form.passwordconfirm><button type=submit>Sign up</button><p class="alert-box warning radius" ng-if=form.error>{{form.error}}</p></form></div></div>'),a.put("partials/topbar.html","<div class=contain-to-grid ng-controller=TopBarCtrl><nav class=top-bar><ul class=title-area><li class=name><h1><a href=projects>OpenEIS</a></h1></li></ul><section class=top-bar-section><ul class=right><li><a>Hello, {{username}}!</a></li><li class=has-form><a class=button ng-click=logOut()>Log out</a></li></ul></section></nav></div>")}]);