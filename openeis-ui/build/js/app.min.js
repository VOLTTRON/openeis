...
angular.module("openeis-ui",["openeis-ui.auth","openeis-ui.projects","openeis-ui.templates","ngAnimate","ngRoute"]).constant("API_URL","/api").constant("ANON_HOME","/").constant("AUTH_HOME","/projects").config(["$routeProvider","$locationProvider","$httpProvider",function(a,b,c){a.otherwise({templateUrl:"partials/404.html"}),b.html5Mode(!0),c.defaults.xsrfHeaderName="X-CSRFToken",c.defaults.xsrfCookieName="csrftoken"}]),angular.module("openeis-ui.auth",["ngResource","ngRoute"]).config(["$routeProvider",function(a){a.when("/",{controller:"LoginCtrl",templateUrl:"partials/login.html"})}]).factory("Auth",["$resource","API_URL","$q",function(a,b,c){var d=null,e="Anonymous",f=a(b+"/auth");return this.username=function(){return e},this.isAuthenticated=function(){var a=c.defer();return null===d?f.get().$promise.then(function(b){d=!0,e=b.username,a.resolve()},function(b){d=!1,a.reject(b)}):d===!1?a.reject({status:403}):d===!0&&a.resolve(),a.promise},this.logIn=function(a){var b=c.defer();return f.save(a).$promise.then(function(a){d=!0,e=a.username,b.resolve()},function(a){b.reject(a)}),b.promise},this.logOut=function(){var a=c.defer();return f.delete().$promise.then(function(){d=!1,e="Anonymous",a.resolve()},function(b){a.reject(b)}),a.promise},this}]).controller("LoginCtrl",["$scope","$location","Auth","AUTH_HOME",function(a,b,c,d){a.form={},a.form.logIn=function(){c.logIn({username:a.form.username,password:a.form.password}).then(function(){b.url(d)},function(b){switch(b.status){case 403:a.form.error="Authentication failed.";break;default:a.form.error="Unknown error occurred."}})}}]).controller("TopBarCtrl",["$scope","Auth","$location","ANON_HOME",function(a,b,c,d){a.username=b.username(),a.logOut=function(){b.logOut().then(function(){c.url(d)})}}]).run(["Auth","ANON_HOME","AUTH_HOME","$location",function(a,b,c,d){a.isAuthenticated().then(function(){d.path()===b&&d.url(c)},function(){d.path()!==b&&d.url(b)})}]),angular.module("openeis-ui.file",[]).directive("fileUpload",["$parse",function(a){return{restrict:"E",template:'<input type="file"><button disabled>Upload</button>',compile:function(b,c){return clickFn=a(c.fileUploadClick),function(a,b){var c=b.find("input"),d=b.find("button");c.on("change",function(){c[0].files?d.prop("disabled",!1):d.prop("disabled",!0)}),d.on("click",function(b){a.$apply(function(){clickFn(a,{$event:b,fileInput:c})})})}}}}]),angular.module("openeis-ui.projects",["openeis-ui.auth","openeis-ui.file","ngResource","ngRoute","mm.foundation","angularFileUpload"]).config(["$routeProvider",function(a){a.when("/projects",{controller:"ProjectsCtrl",templateUrl:"partials/projects.html",resolve:{projects:["Projects",function(a){return a.query()}]}}).when("/projects/:projectId",{controller:"ProjectCtrl",templateUrl:"partials/project.html",resolve:{project:["Projects","$route",function(a,b){return a.get(b.current.params.projectId)}],projectFiles:["ProjectFiles","$route",function(a,b){return a.query(b.current.params.projectId)}]}})}]).factory("Projects",["$resource","API_URL",function(a,b){var c={resource:a(b+"/projects/:projectId",{projectId:"@id"}),get:function(a){return c.resource.get({projectId:a}).$promise},query:function(){return c.resource.query().$promise}};return c}]).factory("ProjectFiles",["$resource","API_URL",function(a,b){var c={resource:a(b+"/files/:fileId"),query:function(a){return c.resource.query({project:a}).$promise},"delete":function(a){return c.resource.delete({fileId:a}).$promise}};return c}]).controller("ProjectsCtrl",["$scope","projects",function(a,b){a.projects=b}]).controller("ProjectCtrl",["$scope","project","projectFiles","$modal","$upload","API_URL","ProjectFiles",function(a,b,c,d,e,f,g){a.project=b,a.projectFiles=c,a.upload=function(c){angular.forEach(c[0].files,function(d){e.upload({url:f+"/projects/"+b.id+"/add_file",file:d}).then(function(b){b.data.top=["These lines were manually injected","into the API's response. The first","few lines of the file will be displayed","once the API actually returns them."].join("\n"),a.projectFiles.push(b.data),a.openModal(b.data),c.val("")})})},a.deleteFile=function(b){g.delete(a.projectFiles[b].id).then(function(){a.projectFiles.splice(b,1)})},a.hasFiles=function(){return a.projectFiles.length},a.openModal=function(a){var b=d.open({templateUrl:"partials/addfile.html",controller:"FileModalCtrl",resolve:{file:function(){return a}}});b.result.then(function(a){console.log(a)},function(a){console.log(a)})}}]).controller("FileModalCtrl",["$scope","$modalInstance","file",function(a,b,c){a.file=c,a.ok=function(){b.close("Clicked OK.")},a.cancel=function(){b.dismiss("Cancelled.")}}]),angular.module("openeis-ui.templates",[]).run(["$templateCache",function(a){"use strict";a.put("partials/404.html","<h2>404 Page not found</h2>"),a.put("partials/addfile.html","<h3>First lines of {{file.file}}</h3><blockquote><pre>{{file.top}}</pre></blockquote><button ng-click=ok()>OK</button> <a class=close-reveal-modal ng-click=cancel()>&#215;</a>"),a.put("partials/login.html",'<div class=row style="margin-top: 200px"><div class="small-8 columns"><h1 style="font-size: 7.5rem; line-height: 1.5">OpenEIS</h1></div><div class="small-4 columns"><form class="panel radius" ng-submit=form.logIn()><input type=text placeholder=Username required ng-model=form.username ng-change="form.error = false"><input type=password placeholder=Password required ng-model=form.password ng-change="form.error = false"><input type=submit value="Log in" class="button radius small"><p class="alert-box warning radius" ng-if=form.error>{{form.error}}</p></form></div></div>'),a.put("partials/project.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=row style="margin-top: 1em"><div class=columns><ul class=breadcrumbs><li><a href=projects>Dashboard</a></li><li>{{project.name}}</li></ul></div></div><div class=row><div class=columns><h2>{{project.name}}</h2><div ng-if=hasFiles()><h3>Project files</h3><ul><li ng-repeat="file in projectFiles">{{file.file}} &mdash; <a href={{file.download_url}} target=_self>Download</a> &mdash; <a ng-click=deleteFile($index)>Delete</a></li></ul></div><h3>Add file</h3><file-upload file-upload-click=upload(fileInput)></file-upload></div></div>'),a.put("partials/projects.html",'<ng-include src="\'partials/topbar.html\'"></ng-include><div class=row style="margin-top: 1em"><div class=columns><ul class=breadcrumbs><li>Dashboard</li></ul></div></div><div class=row><div class=columns><h2>Dashboard</h2></div></div><div class=row id=main><div class=columns><ul ng-if=projects><li ng-repeat="project in projects"><a ng-href=projects/{{project.id}}>{{project.name}}</a></li></ul></div></div>'),a.put("partials/topbar.html","<div class=contain-to-grid ng-controller=TopBarCtrl><nav class=top-bar><ul class=title-area><li class=name><h1><a href=projects>OpenEIS</a></h1></li></ul><section class=top-bar-section><ul class=right><li><a>Hello, {{username}}!</a></li><li class=has-form><a class=button ng-click=logOut()>Log out</a></li></ul></section></nav></div>")}]);